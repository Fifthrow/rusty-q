...
impl<'a> KVal<'a> {
    pub unsafe fn new(k: *const K) -> KVal<'a> {
        let k = &*k;
        match k.t {
            ...
            -128 => {
                let err = {
                    ...
                    buf.clone_from_slice(&(*k).union[0..8]);
                    ffi::CStr::from_ptr(std::mem::transmute(buf))
                };
            ...
            99 => {
                ...
                KVal::Dict(Box::new(KVal::new(slice[0])), Box::new(KVal::new(slice[1])))
            }
            100 => KVal::Function,
            _ => KVal::Unknown,
        }
...
pub fn kerror(err: &str) -> &'static K {
    ...
    let msg = map
        .entry(err.to_string())
        .or_insert_with(|| ffi::CString::new(err).unwrap());
    let ptr = msg.as_ptr();
